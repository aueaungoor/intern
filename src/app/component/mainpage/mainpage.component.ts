...
export class MainpageComponent implements OnInit {
  ...
  allSports = ['Football', 'Basketball', 'Volleyball', 'Tennis', 'Badminton', 'Table Tennis', 'Running', 'Swimming', 'Cycling'];
  allCareers = ['Developer', 'Designer', 'Project Manager', 'Teacher', 'Doctor', 'Engineer', 'Architect', 'Lawyer', 'Data Scientist'];
...
loadAccountspadding(): void {
  ...
  getSport(account: Account): void {
    ...
    this.http.post<SportResponse>('http://localhost:8080/accounts/getSport', account).subscribe({
      next: (res) => {
        console.log("sport",res);
        this.modalType = 'sport';
        this.modalData = res.data.map(item => ({ name: item.sportname , id: item.idaccountSport  }));
        this.showModal = true;
      },
  ...
  getCareer(account: Account): void {
    this.selectedAccount = account;
    this.http.post<CareerResponse>('http://localhost:8080/accounts/getCareer', account).subscribe({
      next: (res) => {
        this.modalType = 'career';
        this.modalData = res.data.map(item => ({ name: item.careername,  id: item.idaccountCareer }));
        this.showModal = true;
      },
  ...
  deleteItem(item: any, type?: 'sport' | 'career', id?: number): void {
    ...
    if (this.modalType === 'career') {
      this.http.delete(`http://localhost:8080/accounts/deleteCareer/${id}`, { responseType: 'text' })
  .subscribe({
    next: (res) => {
    ...
    error: (err) => {
      ...
      this.http.delete(`http://localhost:8080/accounts/deleteSport/${id}` ,{ responseType: 'text' })
      .subscribe({
        next: (res) => {
          console.log('✅ :', res); // <<-- res คือข้อความ string จาก backend
          alert(res); // แสดงข้อความที่ backend ส่งกลับ
  ...
  submitAddSelected(type: 'sport' | 'career' ): void {
    ...
    const requests = this.selectedToAdd.map(name => {
      const body = this.modalType === 'sport'
        ? { sportname: name, account: this.selectedAccount }
        : { careername: name, account: this.selectedAccount };
      const url = `http://localhost:8080/accounts/add${this.modalType.charAt(0).toUpperCase() + this.modalType.slice(1)}`;
      return this.http.post(url, body).toPromise();
    });

    Promise.all(requests).then(() => {
      alert(`เพิ่ม${this.modalType === 'sport' ? 'กีฬา' : 'อาชีพ'}ทั้งหมดสำเร็จ`);
      this.closeModal();
      this.loadAccountspadding();
    }).catch(err => {
  ...
  formeditaccount(idaccount: number): void {
    this.http.get<Account>(`http://localhost:8080/accounts/${idaccount}`).subscribe({
      next: (res) => {
        ...
        if(res.pathpicture)
        {
          this.http.get('http://localhost:8080/accounts/base64' , {
            ...
            if (this.editAccount) {
              this.editAccount.pathpicture = 'data:image/png;base64,' + base64;
            }
  ...
  submitEdit(): void {
    ...
    this.http.put(`http://localhost:8080/editaccount/editaccount/${id}`,this.editAccount).subscribe({
      next: () => {
  ...
  deleteAccount(): void {
    ...
    this.http.delete(`http://localhost:8080/accounts/${this.accountIdToDelete}`, { responseType: 'text' }).subscribe({
      next: () => {
        alert('ลบสำเร็จ');
        this.cancelDelete();
        this.loadAccountspadding();
...
searchAccounts(): void {
  ...
  this.http.post<AccountResponse>(`http://localhost:8080/accounts/search`,this.accountsearch)
    .subscribe({
      next: (res) => {
        console.log(res);
