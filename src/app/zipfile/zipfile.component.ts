...
export class ZipfileComponent {
  ...
  onFolderSelect(event: any) {
    ...
    if (files.length > 0) {
      ...
      this.sourceFolder = `/home/aueaungoorn/${folderName}`; 
  ...
  submitZip() {
    ...
    const url = `http://localhost:8080/accounts/download?sourceFolder=${encodeURIComponent(this.sourceFolder)}&zipName=${encodeURIComponent(this.zipName)}`;
